...
public ResponseEntity<String> proxyUsaJobs(@RequestParam Map<String, String> params) {
    try {
        ...
        // Check if content-type is JSON (allow charset suffix)
        boolean isJson = contentType != null && 
                         contentType.toLowerCase(Locale.ROOT).startsWith("application/json");

        // Check if body starts with HTML
        boolean isHtml = body.trim().startsWith("<");

        if (!isJson || isHtml) {
            System.err.println("USAJobs returned non-JSON content. ContentType: " + contentType + 
                              ", StartsWithHtml: " + isHtml);
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body("{\"SearchResult\":{\"SearchResultItems\":[],\"SearchResultCount\":0},\"warning\":\"USAJobs returned non-JSON\"}");
        }
        ...
        try {
            ...
            // If parsing fails, surface 503 so frontend falls back
            System.err.println("USAJobs JSON dedupe failed: " + ex.getMessage());
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .contentType(MediaType.APPLICATION_JSON)
...
private ResponseEntity<String> aggregateUsaJobs(Map<String, String> baseParams, int desiredRpp) {
    try {
        ...
        while (needed > 0 && safety++ < 50) {
            ...
            if (contentType == null || !contentType.toLowerCase(Locale.ROOT).contains("application/json") || body.trim().startsWith("<")) {
                return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"SearchResult\":{\"SearchResultItems\":[],\"SearchResultCount\":0},\"warning\":\"USAJobs returned non-JSON during aggregation\"}");
            }

            JsonNode root;
            try { root = m.readTree(body); } catch (Exception parseEx) {
                return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"SearchResult\":{\"SearchResultItems\":[],\"SearchResultCount\":0},\"warning\":\"USAJobs JSON parse error during aggregation\"}");
            }
        ...
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .contentType(MediaType.APPLICATION_JSON)
                .body("{\"SearchResult\":{\"SearchResultItems\":[],\"SearchResultCount\":0},\"warning\":\"USAJobs aggregation failed\"}");
    }
}           

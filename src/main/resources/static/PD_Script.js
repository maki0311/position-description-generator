...
async function renderPositionTitleField() {
    ...
    if (formState.jobSeries === '0301') {
        ...
        // Set state before rendering so validation can enable the continue button
    ...
    try {
        ...
        if (input && list) {
            ...
            // Ensure validation runs on first render (when returning to the step)
            if (typeof window.checkComplete === 'function') window.checkComplete();
        }

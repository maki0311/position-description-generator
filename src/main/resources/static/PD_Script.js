...
async function fetchJobSeriesFromAPI(page = 1, allJobs = []) {
    let preferredRpp = 2250;      // your desired page size
    const fallbackRpp = 500;      // safe fallback if upstream rejects large pages
    ...
    try {
        ...
        // Track seen IDs for cross-page dedupe
        ...
        while (pagesFetched < maxPages) {
            ...
            // If API returned nothing on large rpp but claims there are results, retry with fallback
            ...
            // De-duplicate by MatchedObjectId across pages
            if (items.length) {
            ...
            // Stop when we've collected as many as the API says exist or no growth
...
async function fetchPositionTitlesFromAPI(jobSeries) {
    ...
    try {
        ...
        // Extract unique, normalized position titles
        const titles = new Map(); // key: normalized lower-case, value: display title (normalized)
        ...
        if (titlesArray.length === 0) {
            ...
            return [normalizePositionTitle(jobSeriesData[jobSeries]?.title || 'Position Title')];
        }
...
// UPDATED: Remove auto-fill logic - leave blank by default
function autofillSupervisoryLevelFromTitle(positionTitle) {
    if (!positionTitle || typeof positionTitle !== 'string') {
        // Don't set any default value
        return;
    }
}
...
function renderJobClassificationStep(content) {
    ...
    continueBtn.onclick = function() {
        if (!continueBtn.disabled) {
            // CRITICAL: Save any form state before navigating
...
function renderUnknownSeriesStep(content) {
    ...
    if (continueUnknownSeriesBtn) {
        continueUnknownSeriesBtn.onclick = () => {
...
// Update renderDutiesStep to ALWAYS restore from formState
function renderDutiesStep(content) {
    ...
    // Previous button - ALWAYS save duties before leaving
    ...
    if (continueBtn) {
        continueBtn.onclick = function() {
            ...
            if (!continueBtn.disabled && formState.gradeAccepted) {
                // CRITICAL: Save duties before continuing
                syncFormStateFromInputs();
...
function syncFormStateFromInputs() {
    ...
    console.log('Synced formState.duties:', formState.duties);
}

...
async function renderPositionTitleField() {
    ...
    // Helpers: strong normalization + smart title-case (with basic acronym preservation)
    const ACRONYMS = new Set(['IT','HR','GS','USA','US','OPM','FBI','IRS','VA','DOD','DOE','DHS','HHS','NIH','NSA','NIST','USCIS','CBP','ICE','TSA','CMS','USGS','USPTO']);
    ...
    function smartTitleCase(str) {
        ...
        const minor = new Set(['a','an','and','as','at','but','by','for','in','of','on','or','the','to','vs','via','with']);
        const tokens = str.normalize('NFKC').split(/\s+/);
        const last = tokens.length - 1;
        const tc = tokens.map((tok, i) => {
            // Preserve separators inside tokens
            const parts = tok.split(/([\/\-])/);
            const cased = parts.map((p, idx) => {
                if (p === '/' || p === '-') return p;
                const core = p.replace(/^[^A-Za-z0-9]+|[^A-Za-z0-9]+$/g, '');
                const lead = p.slice(0, p.indexOf(core) >= 0 ? p.indexOf(core) : 0);
                const trail = core ? p.slice((p.indexOf(core) >= 0 ? p.indexOf(core) : 0) + core.length) : '';
                if (!core) return p;
                const upper = core.toUpperCase();
                const lower = core.toLowerCase();
                if (ACRONYMS.has(upper) || (core.length >= 2 && core === upper && /[A-Z]/.test(core))) {
                    return lead + upper + trail;
                }
                if (i !== 0 && i !== last && minor.has(lower)) {
    ...
    // Special case: 0301 gets a free text input (persist across steps)
    if (formState.jobSeries === '0301') {
        console.log('Rendering free text input for 0301');

        // Restore from formState, else localStorage
        const stored0301 = localStorage.getItem('pd.positionTitle.0301') || '';
        const currentValue = (formState.positionTitle && formState.positionTitle.trim().length > 0)
        ...
        if (input) {
            ...
            input.addEventListener('input', function() {
                formState.positionTitle = this.value;
                localStorage.setItem('pd.positionTitle.0301', this.value || '');
            ...
            // Title-case and persist on blur
            input.addEventListener('blur', function() {
                const cased = smartTitleCase(this.value);
                this.value = cased;
                formState.positionTitle = cased;
                localStorage.setItem('pd.positionTitle.0301', cased || '');
                autofillSupervisoryLevelFromTitle(cased);
            });
        }
        return;
    }
    ...
    try {
        ...
        // Strong de-duplication (case/punctuation-insensitive) and title-case display
        const uniqueMap = new Map();
        (Array.isArray(titles) ? titles : []).forEach(t => {
            const key = normalizeKey(t);
            if (!uniqueMap.has(key)) uniqueMap.set(key, smartTitleCase(t));
        });
        const uniqueTitles = Array.from(uniqueMap.values());

        console.log('Fetched titles (deduped):', uniqueTitles.length);
        ...
        const initial = formState.positionTitle ? smartTitleCase(formState.positionTitle) : '';
        group.innerHTML = `
            <label class="form-label" for="positionTitleInput">Position Title <span style="color:#ef4444;">*</span></label>
            <div class="dropdown-wrapper">
                <input class="form-input" id="positionTitleInput" autocomplete="off"
            ...
            <div style="color:#10b981; font-size:0.9em; margin-top:0.5rem;">
                <i class="fas fa-check-circle"></i> Loaded ${uniqueTitles.length} position titles
            </div>
        `;
        ...
        if (input && list) {
            ...
            function showList() {
                const term = (input.value || '').trim().toLowerCase();
                const filtered = uniqueTitles.filter(t => t.toLowerCase().includes(term));
                list.innerHTML = filtered.length
                    ? filtered.map(t => `<div class="dropdown-item" data-value="${t}">${t}</div>`).join('')
                    : `<div class="dropdown-item disabled">No results found</div>`;
                list.style.display = 'block';
            ...
            input.addEventListener('blur', () => {
                ...
                const cased = smartTitleCase(input.value);
                input.value = cased;
                formState.positionTitle = cased;
                autofillSupervisoryLevelFromTitle(cased);
            });
            ...
            list.addEventListener('mousedown', function(e) {
                if (e.target.classList.contains('dropdown-item') && !e.target.classList.contains('disabled')) {
                    const chosen = smartTitleCase(e.target.dataset.value || '');
                    input.value = chosen;
...
function renderJobClassificationStep(content) {
    ...
    continueBtn.onclick = function(e) {
        ...
        if (!continueBtn.disabled) {
            // Save any current duties before navigating
            if (typeof syncFormStateFromInputs === 'function') {
                syncFormStateFromInputs();
            }
            
            if (formState.usedAIRecommendation) {
                // CRITICAL: Mark step 4 as visited when skipping it via AI recommendation
                if (!window.visitedSteps) window.visitedSteps = new Set();
